...
window.BioSupabase = (function() {
  ...
  /**
   * Update the current user's password (used by the password recovery flow
   * after the user clicks the reset link and lands on the login page).
   * @returns {Promise<{data: {user}, error}>}
   */
  function updatePassword(newPassword) {
    return init().then(function(c) {
      return c.auth.updatePassword({ password: newPassword });
    });
  }
