...
window.BioData = (function() {
  ...
  function buildDefaultSpeciesRegistry() {
    ...
    function addEntry(commonName, scientificName) {
      ...
      registry.push({
        ...
        baseline_count: null,        // admin override (null = auto-derived)
  ...
  function buildDefaultSites() {
    return [
      { id: 'site_001', name: 'The CBU Nature Park', habitat_type_default: 'Woodland', established: null },
  ...
  function resolveSiteId(candidate) {
    ...
    var match = sites.find(function(s) { return (s.name || '').toLowerCase() === lower; });
    return match ? match.id : null;
  }
  ...
  function hydrateSupabaseProfile(sbUser) {
    ...
    return BioSupabase.ready().then(function(client) {
      return client.from('profiles').select('id, full_name, role, institution_name').eq('id', sbUser.id).maybeSingle();
    }).then(function(result) {
      if (result.error) throw result.error;
      _supabaseProfileCache = result.data || null;
  ...
  // =====================================================
  var HABITAT_TYPES = [
    'CBU Nature Park - Grassland + Woodland (Mixed)',
    'The CBU - Urban'
  ];
