...
window.BioData = (function() {
  ...
  // Focus areas are the two places the current dataset can actually describe.
  // The site registry may also contain historical survey sections (kind=zone),
  // but those are not focus areas and must never leak into a focus-area picker.
  var focusAreaDefinitions = [
    { value: 'The CBU Nature Park', label: 'Nature Park', kind: 'park' },
    { value: 'CBU Campus', label: 'Campus', kind: 'campus' }
  ];
  var focusAreas = focusAreaDefinitions.map(function(definition) { return definition.value; });
  ...
  function buildDefaultSites() {
    ...
    // section and put a whole focus area in a section picker.
    return [
      { id: 'site_001', name: 'The CBU Nature Park', short_name: 'Nature Park', habitat_type_default: 'Miombo Woodland', established: null, kind: 'park', active: true, parent_site_id: null, sort_order: null, latitude: null, longitude: null },
      { id: 'site_002', name: 'CBU Campus', short_name: 'Campus', habitat_type_default: 'Urban', established: null, kind: 'campus', active: true, parent_site_id: null, sort_order: null, latitude: null, longitude: null }
    ];
  }
  ...
  sites.forEach(function(site) {
    ...
    // itself in a section picker. Unknown stays unknown until the cloud answers.
    if (site.kind === undefined) site.kind = null;
    if (site.active === undefined) site.active = site.kind !== 'zone';
    ...
    if (Array.isArray(cloudSites)) {
      ...
      var newSites = cloudSites.map(function(row) {
        return {
          ...
          kind: row.kind || null,
          active: row.active == null ? row.kind !== 'zone' : row.active !== false,
  ...
  /* GBIF is retained only as a recoverable historical import. It is not an
     operational ZitBio observation: it has no field effort, is not part of the
     park census, and must not quietly enter counters, reports, or the ecosystem
     model. Keep this rule in the shared data layer so a page cannot forget it. */
  function isOperationalObservation(obs) {
    if (!obs) return false;
    var source = String(obs.source || '').toLowerCase();
    var provenance = String(obs.provenance || '').toLowerCase();
    return source !== 'gbif' && provenance !== 'external';
  }

  function operationalObservations(rows) {
    return (rows || []).filter(isOperationalObservation);
  }

  // LIVE STATE
  clearStaleStorage(); // discard stale biodata_* before loading
  var users = loadFromStorage('users', defaultUsers);
  var observations = operationalObservations(loadFromStorage('observations', defaultObservations));
  ...
  function updateObservation(id, updates) {
    ...
    if (updates.location) {
      ...
      if (updates.location.focus_area != null) {
        var requestedFocusArea = String(updates.location.focus_area || '').trim();
        if (requestedFocusArea && !isFocusArea(requestedFocusArea)) {
          throw new Error('Focus area must be Nature Park or Campus');
        }
        obs.location.focus_area = requestedFocusArea;
      }
  ...
  function getFocusAreas() {
    return focusAreas.slice();
  }

  // UI-facing focus-area options. This intentionally does not return every row
  // in `sites`: child survey sections are valid historical data, but the current
  // dataset has no section-level measurements to support them as selectable
  // focus areas.
  function getFocusAreaOptions() {
    return focusAreaDefinitions.map(function(definition) {
      var match = sites.find(function(site) {
        return site && site.active !== false &&
          String(site.name || '').toLowerCase() === definition.value.toLowerCase();
      });
      return {
        value: definition.value,
        label: (match && (match.short_name || match.name)) || definition.label,
  ...
  function getFocusAreaSites() {
    var options = getFocusAreaOptions();
    return options.map(function(option) {
      return sites.find(function(site) { return site && site.active !== false && site.id === option.siteId; }) || {
        id: option.siteId,
        name: option.value,
  ...
  function isFocusArea(value) {
    var wanted = String(value == null ? '' : value).trim().toLowerCase();
    return focusAreaDefinitions.some(function(definition) {
      return definition.value.toLowerCase() === wanted || definition.label.toLowerCase() === wanted;
    });
  }
  ...
  function seedSurveyPlane(plane) {
    ...
    var surveys = (plane.surveys || []).filter(function (survey) {
      return String(survey && survey.provenance || '').toLowerCase() !== 'external';
    });
    var surveyIds = {};
    surveys.forEach(function (survey) { if (survey && survey.id != null) surveyIds[String(survey.id)] = true; });
    var keepSurveyRow = function (row) { return row && surveyIds[String(row.survey_id)]; };
    surveyPlane = {
      surveys: surveys,
      zones: (plane.zones || []).filter(keepSurveyRow),
      vegetation: (plane.vegetation || []).filter(keepSurveyRow),
      water: (plane.water || []).filter(keepSurveyRow),
      soil: (plane.soil || []).filter(keepSurveyRow),
      sward: (plane.sward || []).filter(keepSurveyRow)
    };
    return surveyPlane;
  }
  ...
  return {
    ...
    getObservations: function() { return observations.filter(isOperationalObservation); },
    isOperationalObservation: isOperationalObservation,
    ...
    seedData: function(seedUsers, seedObservations) {
      ...
      if (Array.isArray(seedObservations) && seedObservations.length > 0) {
        observations = operationalObservations(seedObservations).map(function(o) { return normalizeObservation(o); });
        saveToStorage('observations', observations);
      }
        ...
        return observations.filter(function(obs) {
            ...
            if (filters.focusArea) {
                ...
                if (/nature park/.test(want)) {
                    ...
                    // Campus / university bucket: campus focus areas, excluding the
                    // park. Mirrors the lenient matching in resolveSiteId() so every
                    // stored variant resolves to the bucket.
                    if (isPark) return false;
    ...
    getFocusAreaOptions: getFocusAreaOptions,
    getFocusAreaSites: getFocusAreaSites,
