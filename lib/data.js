...
window.BioData = (function() {
  ...
  /**
   * Human-readable account code for a profile, e.g. "ADMIN-004" or "FO-3A1C".
   *
   * Cloud profiles carry a uuid, which is unreadable in the header dropdown and
   * on Settings → Account Information, so a uuid is shortened to its first four
   * hex characters and legacy numeric ids are padded. Returns an em dash rather
   * than a guess when there is nothing to show — an account code is an identity,
   * and a made-up one (the old static "ADM-001" placeholder) is worse than none.
   *
   * Shared by the user dropdown and Settings so the same person is never shown
  ...
  //
  // `role` is deliberately null while the profile is still unknown. The previous
  // `|| 'field_officer'` fallback invented a role for a real person, which is how
  // an admin ended up labelled "Role: Field Officer" in the user dropdown: the
  // shell mounts before this module's own profile fetch lands, and the invented
  // value was never corrected afterwards (#86). "Unknown" must stay unknown —
  // the verified role is the route guard's, so use that for anything that needs
  // to decide or display, never this cache.
  //
  // `id` is the Auth user's uuid, and display codes such as the dropdown's
  // account number are derived from it (see formatAccountId).
  ...
  /**
   * Adopt the route guard's verified profile as this session's identity.
   *
   * The guard resolves the role from the signed access token (or the profiles
   * row) *before* it reveals the page, so handing that verdict over here means
   * every reader of getSession() — the user dropdown, Settings → Account — shows
   * the verified role on the first paint instead of waiting for this module's own
   * background fetch (#86). Display only: nothing in this file grants privilege
   * from a session, and the guard never reads the value back out.
   */
  function seedVerifiedProfile(profile) {
    if (!profile) return null;
    // Offline the guard downgrades the *privilege* to field officer on purpose
    // (#40). That is a statement about what the app may do, not about who the
    // person is, so it is not adopted as identity: the persisted session keeps
    // the last role that was actually verified.
  ...
  function getSession() {
    ...
    if (window.BioSupabase && window.BioSupabase.isConfigured()) {
      ...
      if (sb && sb._user) {
        ...
        // The `session:changed` publish matters: the shell has already mounted by
        // the time this resolves, so anything showing the role needs the nudge to
        // repaint, otherwise the pre-hydration value is what the user keeps (#86).
  ...
  return {
    ...
    // Verified identity adopted from the route guard (role/name/id display)
