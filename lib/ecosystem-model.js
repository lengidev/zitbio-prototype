...
(function (root, factory) {
  ...
  var MODEL_VERSION = 'cbu-demo-decision-v6';
  ...
  var EDGES = [
    ...
    },
    {
      ...
      conditions: 'The demo activates the adverse branch when the 30-day forage balance fails; a buffer shorter than 60 days remains neutral rather than being called low competition.',
      source: SOURCES.AFRICAN_HERBIVORE_PARTITION
    },
    {
    ...
    },
    {
      ...
      conditions: 'The demo activates this pathway only when rainfall is high and the ground screen is exposed; rainfall alone does not assert high erosion.',
      source: SOURCES.NRCS_GROUND_COVER
    },
    {
      from: 'rainChemistry', to: 'forage', direction: 'up', directOrIndirect: 'indirect',
      mechanism: 'Rain chemistry within the reference state avoids the deposition stress applied to grass regrowth in the acidic demo.',
      conditions: 'The demo treats pH 4.2 as an acidic-deposition stress and pH 5.6 as typical rain; local buffering and exposure duration remain uncalibrated.',
  ...
  function conditionFor(edge, context, water, deterministic) {
    ...
    if (key === 'zebra>soil') {
      ...
      if (context.soil === 'stable') return { signs: [0], note: 'The demo assumes moderate protective cover, so this direct soil pathway is held stable.' };
    ...
    if (key === 'zebra>waterQuality') {
      ...
      if (context.bank === 'managed') return { signs: [0], note: 'The demo assumes moderate bank disturbance without activating a direct water-quality decline.' };
    ...
    if (key === 'woody>forage') {
      ...
      if (context.woody === 'balanced') return { signs: [0], note: 'The demo assumes mixed woody cover with no net grass effect.' };
    ...
    if (key === 'waterQuality>usableWater') {
      return water.drinkingConcern
        ? { signs: deterministic ? [1] : [0, 1], note: 'Suspected contamination or bloom: the demo restricts use of the water point pending verification.' }
    ...
    if (key === 'competition>soil') {
      ...
      if (context.soil === 'stable') return { signs: [0], note: 'Moderate ground cover holds this crowding-to-soil pathway stable in the demo.' };
  ...
  function deriveEffects(speciesChanges, driver, context, water, demo) {
    ...
    if (demo) {
      ...
      if (water.availability === 'adequate') seed('water', 0, 'Water availability is held at the entered reference level; it is not re-estimated from season.', true);
    }
  ...
  function resolveEffectsFromDemo(effects, edgeStates, demo) {
    ...
    effects.forEach(function (effect) {
      ...
      effect.decision = {
        ...
        basis: 'Resolved demo calculation',
  ...
  function scenarioDecision(effects, water, errors, demo) {
    ...
    if (water.drinkingConcern || water.availability === 'dry' || water.availability === 'limited') {
      ...
      if (water.availability === 'limited') result.reasons.push('The selected water point has limited access; future recharge does not remove the current constraint.');
      ...
      result.action = 'Do not add animals until the entered forage and usable-water screens support the herd for the 30-day demo horizon.';
      ...
      result.action = 'Keep the entered herd unchanged and collect the missing water readings before using the demo to support expansion.';
      ...
      result.action = 'The explicit demo screens support the entered herd for 30 days; repeat the same measurements before any expansion.';
    ...
    if (demo) {
      result.reasons.push('Demo result: ' + demo.components.forage.label + '; ' + demo.components.water.label + '; ' + demo.components.usableWater.label + '; ' + demo.components.competition.label + '; ' + demo.components.soil.label + '.');
      ...
      result.reasons.push('The result uses explicit teaching values and published screening rules, not unmeasured pathway voting.');
    ...
    else if (water.provenance === 'demonstration') result.reasons.push('The selected water values are explicit teaching values for this demonstration.');
  ...
  function evaluateScenario(input) {
    ...
    var unknowns = demo ? [
      'All habitat, rainfall and water presets are teaching values. They are explicit so the demo resolves, but they are not CBU park measurements.',
      ...
      'Published Kafue samples are historical regional analogues; demonstration samples are invented. Neither is a current measurement of the park.'
    ];
