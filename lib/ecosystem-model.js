...
(function (root, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = factory(require('./water-quality.js'));
  } else {
    root.BioEcosystem = factory(root.BioWaterQuality);
  }
}(typeof window !== 'undefined' ? window : this, function (waterModel) {
  'use strict';

  var MODEL_VERSION = 'cbu-decision-v4';
  ...
  function conditionFor(edge, context, water) {
    ...
    if (key === 'waterQuality>usableWater') {
      return water.drinkingConcern
        ? { signs: [0, 1], note: 'Suspected contamination or bloom: suitability may be reduced; verify with a targeted sample.' }
   ...
   */
  function deriveEffects(speciesChanges, driver, context, water) {
    var results = {};
    var queue = [];
    ...
    if (water.availability !== 'unmeasured') fixedInputs.water = true;
    ...
    while (queue.length) {
      ...
      outgoing.forEach(function (edge) {
        ...
        var condition = conditionFor(edge, context, water);
    ...
    Object.keys(COMPONENTS).forEach(function (key) { if (!fixedInputs[key] && key !== 'rainfall') resultFor(key); });
  ...
  var CONTEXT_OPTIONS = {
    soil: ['unmeasured', 'exposed', 'nutrientReturn'],
    bank: ['unmeasured', 'disturbed', 'protected'],
    woody: ['unmeasured', 'dense', 'open']
  };
  ...
  function scenarioDecision(effects, water, errors) {
    ...
    if (water.drinkingConcern || water.availability === 'dry') {
    ...
    if (adverse.length) result.reasons.push('Components with an adverse possibility: ' + adverse.map(function (effect) { return effect.label; }).join(', ') + '.');
    if (competing.length) result.reasons.push('Where mechanisms compete or a condition is missing, use the adverse possibility for planning without presenting it as a forecast.');
    ...
    else if (water.provenance === 'demonstration') result.reasons.push('The selected water values are invented for demonstration.');
  ...
  function evaluateScenario(input) {
    ...
    var derived = deriveEffects(speciesChanges, driver, conditions, water);
    var edges = derived.effects;
    edges.forEach(function (effect) {
      effect.decision = decisionForEffect(effect);
    ...
    var decision = scenarioDecision(edges, water, inputErrors);
    var pathways = [];
    edges.forEach(function (effect) {
