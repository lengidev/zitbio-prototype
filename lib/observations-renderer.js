// lib/observations-renderer.js: one configuration-driven renderer for every
// observations table. Each page supplies its own columns, pagination style and row
// actions; the renderer only emits markup and the page wires the behaviour.
...
//   { ariaLabel, active, kind, highlight, options: [{ value, label }] }
// `kind` lets the page route the choice to the right behaviour, and
// `highlight: false` is for a menu that switches what the column shows rather
// than narrowing the rows (an always-on "filtered" colour would be a false
// signal). The renderer emits markup + `data-filter-value`; the page wires it.
...
/** Kept separate so the no-filter path stays a one-liner. */
function buildTableHeadCell(col) {
    var f = col.filter;
    if (!f) return col.label;

    var active = f.active || '';
    var filtered = (active && f.highlight !== false) ? ' is-filtered' : '';
    var html = '<div class="th-filter">' +
        '<button type="button" class="th-filter-btn' + filtered + '"' +
    ...
    for (var i = 0; i < options.length; i++) {
        ...
        html += '<button type="button" class="th-filter-option' + (isActive ? ' is-active' : '') + '"' +
            ...
            ' data-filter-value="' + escapeHtmlObs(o.value || '') + '"' +
            (f.kind ? ' data-filter-kind="' + escapeHtmlObs(f.kind) + '"' : '') + '>' + o.label + '</button>';
    }
...
// Delegates to BioEscape, the single implementation. The version that lived here
// ran four self-replacements, so callers looked protected and were not.
...
// Two pagination skins share this renderer: 'bar' (Analytics: info left, page
// numbers and prev/next right) and 'centered' (Observations: external prev/next).
...
function renderPagination(paginationSelector, style, currentPage, totalPages, startRecord, endRecord, totalRecords, onPageChange) {
    ...
    if (style === 'bar') {
        ...
        // The observations page owns its own .page-item elements, so only the info
        // text is updated here.
