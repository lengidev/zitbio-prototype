/**
 * BioMonitor — Shared Observations Table Renderer
...
function renderObservationsTable(options) {
    ...
    if (totalRecords === 0) {
        ...
        renderPagination(paginationSelector, paginationStyle, page, totalPages, startRecord, endRecord, totalRecords);
        return { pageData: pageData, totalRecords: totalRecords, totalPages: totalPages, startRecord: startRecord, endRecord: endRecord };
    }
...
// Capitalizes the first letter of each word so names typed in lowercase
// display properly in tables (e.g. "plains zebra" → "Plains Zebra").
// Preserves existing capitalization and non-breaking whitespace.
...
function renderPagination(paginationSelector, style, currentPage, totalPages, startRecord, endRecord, totalRecords) {
    var container = document.querySelector(paginationSelector);
    if (!container) return;

    if (style === 'bar') {
        renderBarPagination(container, currentPage, totalPages, startRecord, endRecord, totalRecords);
...
function renderBarPagination(container, currentPage, totalPages, startRecord, endRecord, totalRecords) {
