...
window.BioAnalytics = (function() {
  ...
  function ecosystemInsights(observations, options) {
    ...
    // Scientific names present in the data.
    var present = {};
    list.forEach(function(o) {
      var sci = o && o.species_details && o.species_details.scientific_name;
      if (sci) present[sci.toLowerCase()] = true;
    });
    ...
    Object.keys(ecology).forEach(function(sci) {
      ...
      // Condition drives the dynamic impact text: below-baseline warning
      // shows the 'down' consequence, an increasing trend shows 'up', trees
      // with no records are assumed present, and everything else is healthy.
