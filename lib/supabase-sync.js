...
window.BioSync = (function() {
    ...
    return ensureReady().then(function(client) {
      ...
      return window.BioSupabase.getSession().then(function(sessionResult) {
        ...
        var observationsPromise = client.from('observations')
          .select('*')
          .is('deleted_at', null)
          // Keep legacy rows whose source is still null, while excluding the
          // explicitly external GBIF import.
          .or('source.is.null,source.neq.gbif')
        ...
        // The query guard keeps the common path small; the client-side guard is
        // intentional defence in depth for old PostgREST deployments and local
        // fixtures that do not apply the filter above.
        var cloudObservations = (obsResult.data || [])
          .filter(function (row) { return String(row.source || '').toLowerCase() !== 'gbif' && String(row.provenance || '').toLowerCase() !== 'external'; })
          .map(mapObservation);
  ...
  function loadSurveyPlane() {
    return ensureReady().then(function(client) {
      // Wildlife monitoring is the v1 product. Optional environmental tables must
      // not prevent its read path from loading when a partial deployment lacks a
      // later module or its RLS policy.
      ...
      function optional(table) {
        return client.from(table).select('*').then(function(result) {
          return result && !result.error ? (result.data || []) : [];
        }).catch(function() { return []; });
      }
      return Promise.all([core, optional('vegetation_readings'), optional('water_readings'), optional('soil_readings'), optional('vegetation_species')]);
    }).then(function(results) {
      var core = results[0];
      var plane = {
        surveys: (core[0].data || []).filter(function (row) { return String(row.provenance || '').toLowerCase() !== 'external'; }),
        zones: core[1].data || [],
      ...
      var surveyIds = {};
      plane.surveys.forEach(function (survey) { if (survey && survey.id != null) surveyIds[String(survey.id)] = true; });
      ['zones', 'vegetation', 'water', 'soil', 'sward'].forEach(function (key) {
        plane[key] = plane[key].filter(function (row) { return row && surveyIds[String(row.survey_id)]; });
      });
  ...
  function submitSurvey(payload) {
    ...
    if (surveyRow.survey_type !== 'wildlife_census') {
      return Promise.reject(new Error('Only wildlife census walks are enabled in ZitBio v1.'));
    }
    // The RPC owns the parent and every child write in one database transaction.
    // Do not fall back to the former browser-side Promise.all: a retry after one
    // failed child would otherwise leave a misleading half-written walk.
    return ensureReady().then(function(client) {
      return client.rpc('submit_wildlife_survey', { p_payload: {
        surveyRow: surveyRow,
        zoneRows: payload.zoneRows || [],
