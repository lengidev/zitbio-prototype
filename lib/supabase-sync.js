...
window.BioSync = (function() {
  ...
  function mapProfile(row) {
      // Shared formatter (#46 / #62). lib/date.js is loaded by every page that
      // renders users, but guard anyway: supabase-sync is also reachable from
      // the login page, which does not.
      // Medium style, not numeric: "8/24/2026" is ambiguous to a DD/MM reader,
      // and it contradicted the observations table one click away.
  ...
  // session — /auth/v1/user was a network round trip on every page load for an
  // id the persisted session already carries).
