/**
 * ZitBio — Shared toast surface
 *
 * Transient, colour-coded confirmation for an action whose result would
 * otherwise only be visible inside the surface that just handled it — or, worse,
 * nowhere at all once that surface closes.
 *
 * Why this exists rather than a call site per page: the app had three different
 * situations and none of them worked. `showToast()` was defined only on the
 * field-officer page; the toast CSS lived in `settings.css`; and several admin
 * pages called `showToast(...)` behind a `typeof === 'function'` guard, so on
 * those pages the calls silently did nothing at all. Same feature, three
 * half-implementations.
 *
 * Usage:
 *   BioToast.show('Zebra approved and logged.', 'success');
 *   BioToast.error('Could not flag Zebra — permission denied.');
 *
 * `window.showToast(message, type)` is also provided, because existing call
 * sites (lib/auth-guard.js, users.js, settings.js) already use that name.
 */
(function () {
  'use strict';
  ...
  /** The container is created on first use, so including this file is enough. */
   ...
   * @param {string} message  Plain text. Never HTML — set via textContent.
   ...
   * a <use href="#i-close"> would resolve to nothing on some of them. Grid is
   * 24x24 to match its own viewBox.
   */
  ...
  /**
   * Remove a toast, honouring the exit transition before detaching it.
   * Guarded so a manual dismissal and the auto-dismiss timer cannot both run.
   */
