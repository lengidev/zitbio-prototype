...
(function (root, factory) {
  ...
  var FIELDS = [
    ...
    { key: 'referenceTurbidity', label: 'Reference Turbidity', unit: 'NTU', min: 0, max: 100000, step: '0.1' },
  ...
  /* A sample label states what the sample is. The page already frames the whole
     workspace as a scenario, so the labels never announce themselves as
     examples. Only the four defined states are offered on the page; the
     published regional rows and the free-entry row stay in the model, marked
     selectable: false, because the historical evidence and the provenance
     rules that reference them are still worth testing. */
  var SAMPLES = [
    { id: 'none', label: 'No Sample', provenance: 'none', values: {} },
    { id: 'demo-reference', label: 'Screened Water', provenance: 'demonstration', values: { ph: 7.2, oxygen: 7, turbidity: 4, referenceTurbidity: 4, availability: 'adequate', contamination: 'notObserved', bloom: 'notObserved' }, note: 'Values chosen for this scenario, not a field sample, and not proof of safe drinking water.' },
    { id: 'demo-oxygen', label: 'Low Oxygen And Cloudy Water', provenance: 'demonstration', values: { ph: 7.2, oxygen: 2, turbidity: 40, referenceTurbidity: 4, availability: 'adequate', contamination: 'notObserved', bloom: 'notObserved' }, note: 'Values chosen for this scenario. Low oxygen affects aquatic organisms; it does not establish toxicity to mammals.' },
    { id: 'demo-contamination', label: 'Suspected Contaminant', provenance: 'demonstration', values: { ph: 5.5, oxygen: 4, turbidity: 30, referenceTurbidity: 4, availability: 'adequate', contamination: 'suspected', bloom: 'notObserved' }, note: 'The contamination flag is an entered scenario assumption, not inferred from pH alone.' },
    { id: 'kafue-2018-07-31', selectable: false, label: 'Kafue · 31 Jul 2018 · published', provenance: 'regional', sampledAt: '2018-07-31', site: 'Kafue River, sampling point 454', source: SOURCES.KAFUE_SAMPLES, values: { ph: 8.3, copper: 0.01, tss: 7 }, note: 'Historical regional analogue, not CBU park water. Appendix 9 reports pH 8.30, dissolved Cu 0.01 mg/L and TSS 7 mg/L. Oxygen and turbidity were not reported in this row; TSS is not NTU.' },
    { id: 'kafue-2018-07-24', selectable: false, label: 'Kafue · 24 Jul 2018 · published', provenance: 'regional', sampledAt: '2018-07-24', site: 'Kafue River, sampling point 454', source: SOURCES.KAFUE_SAMPLES, values: { ph: 6.8, copper: 0.02, tss: 19 }, note: 'Historical regional analogue, not CBU park water. Appendix 9 reports pH 6.80, dissolved Cu 0.02 mg/L and TSS 19 mg/L. Missing parameters are left blank.' },
    { id: 'custom', selectable: false, label: 'Custom scenario / enter sample values', provenance: 'user-entered', values: {}, note: 'User-entered scenario values. Record the site and sample date if transcribing measurements; this does not verify the sample.' }
  ];

  /* Provenance words a reader sees beside the selected sample. */
  var PROVENANCE_LABELS = {
    none: 'no sample selected',
    demonstration: 'values chosen for this scenario',
    regional: 'published regional sample',
    'user-entered': 'values entered for this scenario'
  };

  var OVERRIDE_LABELS = { availability: 'water access', bloom: 'algal bloom' };

  /* Short phrases for the states a water point can be in, so prose never prints
     the stored token ('adequate', 'notObserved') at a reader. */
  var AVAILABILITY_LABELS = {
    unmeasured: 'not assessed',
    adequate: 'enough water',
    limited: 'limited water',
    dry: 'a dry water point'
  };
  ...
  function assess(input) {
    ...
    /* Water access and the bloom indication stay editable on top of a selected
       sample so the dry-access and suspected-bloom pathways remain reachable.
       A deviation is reported rather than absorbed, because the sample label
       would then describe only part of the input. */
    var overrides = ['availability', 'bloom'].filter(function (key) {
      var preset = sample.values[key] == null ? 'unmeasured' : sample.values[key];
      var entered = input[key] == null || String(input[key]).trim() === '' ? 'unmeasured' : input[key];
      return String(entered) !== String(preset);
    });
    ...
    if (contamination === 'unmeasured' || bloom === 'unmeasured') gaps.push('Contamination and bloom were not recorded for this sample.');
    ...
    var label = errors.length ? 'Correct water inputs' : severity >= 3 ? 'Act on water risk' : severity >= 1 ? 'Investigate water stress' : sample.id === 'demo-reference' ? 'Reference screen met' : hasInput ? 'Complete water verification' : 'Collect a water sample';
    ...
    return {
      ...
      provenanceLabel: PROVENANCE_LABELS[provenance] || provenance,
      ...
      overrides: overrides,
      overrideNote: overrides.length ? 'Set by hand: ' + overrides.map(function (key) { return OVERRIDE_LABELS[key]; }).join(' and ') + '. These no longer follow the selected sample.' : '',
