...
function renderNotifications() {
  ...
  cloud.concat(local).forEach(function(n) {
    if (!n || !n.id) return;
    // These were generated by the withdrawn sighting-average rule. They are
    // not valid population alerts and would keep presenting a stable herd as
    // "low" after the report has been corrected.
    if (n.type === 'population_warning') return;
...
function runAnalyticsEvaluation() {
  // A single sighting is not a park population estimate. The earlier rolling
  // average compared arbitrary observation rows with a fixed herd baseline and
  // could therefore call a stable herd of three zebras "low" after one partial
  // sighting. Ecological State now owns population interpretation: it requires
  // approved, recent, zone-scoped census coverage before it says anything.
  // Do not create bell alerts from the generic Analytics dataset.
  return;
}
