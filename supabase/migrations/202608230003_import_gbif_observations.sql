-- ============================================================================
-- ZitBIO — Import GBIF Observations (Copperbelt University)
-- ============================================================================
-- Source : src/gbif_occurrences.csv (GBIF download, accessed 2026-08-23)
-- Filter : eventYear 2016–2026 (10-year window)
-- Count  : 173 records
-- Mapping notes:
--   - source = 'gbif' (provenance marker)
--   - verification_status = 'Approved' (GBIF/eBird/Pl@ntNet/Observation.org
--     records are already vetted/validated)
--   - count = 1 per occurrence (CSV carries no individual counts)
--   - habitat_type = 'Urban' (2-option habitat system; every record
--     falls on/near campus, not the Nature Park)
--   - institution_name = full source institution (not abbreviated)
--   - scientific_name = GBIF name with authorship stripped (e.g. "Scopus umbretta")
--   - common_name = mapped vernacular name where known
--   - gbifID preserved in field_notes for traceability
-- Idempotent: re-running inserts 0 new rows (observation_id is unique).
-- ============================================================================
