-- ============================================================================
-- ZitBIO — Seed Initial Observations
-- ============================================================================
-- Seeds the default observations currently defined in lib/data.js
-- (defaultObservations) so the app's dashboard/analytics/map show the same
-- data immediately after migration. Survey window: 14 Jun – 14 Aug 2026,
-- recorded by Lenganji Sinyangwe and Lenson Mulaga within the CBU Nature Park.
-- Counts hover around the research-documented populations (Zebra 3 ·
-- Waterbuck 3 · Puku 5 · Impala 8) so the low-population estimate (mean of
-- recent counts) reads as healthy, not down. All records approved.
--
-- NOTE: user_id is NULL because these seed records pre-date real Supabase
-- Auth users. recorded_by preserves the display names. When team members are
-- created via Supabase Auth, existing observations are NOT re-linked (avoids
-- mis-association); new observations will carry user_id.
-- ============================================================================
